...
class iONLiveCameraVideoCapture: NSObject {
    ...
    {
        let requestManager = RequestManager.sharedInstance
        requestManager.httpManager().DELETE(UrlManager.sharedInstance.getiONLiveVideoUrlWithHlsId(hlsID), parameters: nil,success: { (operation, response) -> Void in
    ...
    func downloadm3u8Video(hlsID hlsID: String, success: ((response: AnyObject?)->())?, failure: ((error: NSError?, code: String)->())?)
    {
        let requestManager = RequestManager.sharedInstance
