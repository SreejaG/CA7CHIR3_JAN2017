...
public class SwiftAlertView: UIView {
    ...
    public func show(view: UIView) {
        ...
        if dimBackgroundWhenShowing == true {
            ...
            let recognizer = UITapGestureRecognizer(target: self, action: #selector(outsideClicked(recognizer:)))
            dimView!.addGestureRecognizer(recognizer)
        }
        if delegate?.responds(to: #selector(SwiftAlertViewDelegate.willPresentAlertView(alertView:))) == true
        {
        ...
        case .FadeIn:
            self.alpha = 0
            UIView.animate(withDuration: appearTime, delay: 0, options: .curveEaseInOut, animations: {
                self.alpha = 1
            }, completion: { (finished) in
                if self.delegate?.responds(to: #selector(SwiftAlertViewDelegate.didPresentAlertView(alertView:))) == true
                {
        ...
        case .FlyFromTop:
            ...
            UIView.animate(withDuration: appearTime, delay: 0, options: .curveEaseInOut, animations: {                 self.frame = tempFrame
            }, completion: { (finished) in
                if self.delegate?.responds(to: #selector(SwiftAlertViewDelegate.didPresentAlertView(alertView:))) == true {
                    self.delegate?.didPresentAlertView!(alertView: self)
                }
        ...
        default:
            ...
            self.alpha = 0.6
            UIView.animate(withDuration: appearTime, animations: {
    ...
    public func dismiss() {
        if self.delegate?.responds(to: #selector(SwiftAlertViewDelegate.willDismissAlertView(alertView:))) == true {
            self.delegate?.willDismissAlertView!(alertView: self)
        }
        
        if dimView != nil {
            UIView.animate(withDuration: disappearTime, animations: {
        ...
        if disappearType == SwiftAlertViewDisappearType.Default {
            self.transform = CGAffineTransform.identity
            UIView.animate(withDuration: disappearTime, delay: 0.02, options: .curveEaseOut, animations: {
                self.alpha = 0
            }, completion: { (finished) in
                self.removeFromSuperview()
                if self.delegate?.responds(to: #selector(SwiftAlertViewDelegate.didDismissAlertView(alertView:))) == true {
    ...
    public func handleClickedButtonAction(action: @escaping (_ buttonIndex: Int) -> (Void)) {
    ...
    private func setUp(title: String?, message: String?, contentView: UIView?, delegate: SwiftAlertViewDelegate?, cancelButtonTitle: String?, otherButtonTitles: [String]?) {
        ...
        NotificationCenter.default.addObserver(self,
                                               selector:#selector(deviceDidRotate(aNotifitation:)),
                                               name: NSNotification.Name.UIApplicationDidChangeStatusBarOrientation,
                                               object: nil)
    }
    
    @objc func deviceDidRotate(aNotifitation: NSNotification) -> Void {
    ...
    private func layoutElementBeforeShowing() {
        ...
        for button in buttons {
            ...
            if !buttonTitleColor.isEqual(UIColor(red: 0, green: 0.478431, blue: 1, alpha: 1)) {
                button.setTitleColor(buttonTitleColor, for: UIControlState.normal)
            }
            button.addTarget(self, action: #selector(SwiftAlertView.buttonClicked(button:)), for: .touchUpInside)
        }
        ...
        let topPartHeight = (contentView == nil) ? (Double(titleTopMargin) + Double(titleLabel.frame.size.height) +  Double(titleToMessageSpacing) + Double(messageLabel.frame.size.height) +  Double(messageBottomMargin)) : Double(contentView!.frame.size.height)
    ...
    func buttonClicked(button: UIButton) {
        ...
        if delegate?.responds(to: #selector(SwiftAlertViewDelegate.alertView(alertView:clickedButtonAtIndex:))) == true
        {
